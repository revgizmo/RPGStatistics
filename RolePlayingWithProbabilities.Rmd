...
What's it look like when we play with Ability Scores:

```{r d6Distribution, echo = FALSE}
barplot(setNames(rep(1/6,6), 1:6), xlab = "Roll Value", ylab = "Probability of Rolling", ylim = c(0,.2), main = "Roll 1d6")
```

So equal odds of rolling 1-6 with each die roll (if they're fair dice)

## Different Ability Score Rules

Each character has 6 Ability Scores: Strength, Dexterity, Constitution, Intelligence, Wisdom, and Charisma.


### Old DnD Rules: 3d6

The old DnD rules were fairly simple: roll 3 six-sided dice (3d6) and add up the total for each of the 6 Ability Scores.

The Distribution of expected scores looked like this:

```{r roll3d6Distribution, echo = FALSE}
prob3d6 = expand.grid(1:6,1:6,1:6)
prob3d6 = rowSums(prob3d6)
prob3d6mean <- mean(prob3d6)
prob3d6median <- median(prob3d6)
prob3d6 <- table(prob3d6)/length(prob3d6)
barplot(prob3d6, xlab = "Roll Value", ylab = "Probability of Rolling", ylim = c(0,0.2), main = "Old Rules: 3d6")
```

This results an approximately normal distribution centered at `r prob3d6mean`.


### Newer Dnd Rules (I'm playing with now): 4d6, drop the lowest, reroll 1s

The Newer Dnd Rules (thatI'm playing with now) are alot more complicated:

1. Roll 4 six-sided dice (4d6)
1. Reroll any 1s (more on this in a minute)
1. Drop the lowest (leaving 3 scores)
1. Add up the total for the 3 remaining dice
1. Repeat for each of the 6 Ability Scores.

For mathematical purposes, I'm going to treat the calculation like this:
* Reroll any 1s == each die is treated as a d5 +1 (scores range from 2 to 6 with equal weights)
* because I'm lazy, I'm going to calculate as 4d5, subtract the lowest, +3

The Distribution of expected scores looked like this:

```{r NewRulesDistribution, echo = FALSE}

probNewRules <- expand.grid(2:6,2:6,2:6,2:6)
probNewRules <- rowSums(probNewRules) + (-1 * apply(probNewRules, 1, FUN=min) )
probNewRulesmean <- mean(probNewRules)
probNewRulesmedian <- median(probNewRules)
probNewRules <- table(probNewRules)/length(probNewRules)

barplot(probNewRules, xlab = "Roll Value", ylab = "Probability of Rolling", ylim = c(0,0.2), main = "Newer Dnd Rules: 4d6, drop the lowest, reroll 1s")

```

This results an dramatically higher scores with a mean of `r probNewRulesmean` and median of `r probNewRulesmedian`.


### A more morderate alternative: 3d6, reroll 1s

A more morderate alternative is a little less complicated:

1. Roll 3 six-sided dice (3d6)
1. Reroll any 1s (more on this in a minute)
1. Add up the total for the 3 dice
1. Repeat for each of the 6 Ability Scores.
...
```{r Alternate1Distribution, echo = FALSE}

probAlternate1 <- expand.grid(2:6,2:6,2:6)
probAlternate1 <- rowSums(probAlternate1) 
probAlternate1mean <- mean(probAlternate1)
probAlternate1median <- median(probAlternate1)
probAlternate1 <- table(probAlternate1)/length(probAlternate1)

barplot(probAlternate1, xlab = "Roll Value", ylab = "Probability of Rolling", ylim = c(0,0.2), main = "A more morderate alternative: 3d6, reroll 1s")

```

This results in scores approximately 1.5 points higher: an approximately normal distribution with a mean of `r probAlternate1mean` and median of `r probAlternate1median`.



### Another more morderate alternative: 4d6, drop the lowest

Another more morderate alternative is also a little less complicated:
...
```{r Alternate2Distribution, echo = FALSE}
library(tidyverse)

probAlternate2 <- expand.grid(1:6,1:6,1:6,1:6)
probAlternate2 <- rowSums(probAlternate2) + (-1 * apply(probAlternate2, 1, FUN=min) )
probAlternate2mean <- mean(probAlternate2)
probAlternate2median <- median(probAlternate2)
probAlternate2 <- table(probAlternate2)/length(probAlternate2)

barplot(probAlternate2, xlab = "Roll Value", ylab = "Probability of Rolling", ylim = c(0,0.2), main = "Another more morderate alternative: 4d6, drop the lowest")

```

This results in scores that are also approximately 1.5 points higher: an approximately normal distribution with a mean of `r probAlternate2mean` and median of `r probAlternate2median`.
...
```{r NewRulesModifierDistribution, echo = FALSE}
...
probNewRules <- probNewRules %>% 
  as_tibble() %>% 
  rename(ability_score = probNewRules,
         probability = n) %>% 
  mutate(ability_score = as.integer(ability_score),
         modifier = floor( (ability_score - 10 ) / 2))

probNewRulesmodifiermean <- mean(probNewRules$modifier)
probNewRulesmodifiermedian <- median(probNewRules$modifier)

probNewRules %>% 
  ggplot(aes(x = modifier, y = probability)) + 
  geom_col() + 
  theme_classic() 
...
This results an dramatically higher scores with a mean of `r probNewRulesmodifiermean` and median of `r probNewRulesmodifiermedian`.
